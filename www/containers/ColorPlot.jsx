...
function ColorPlot(props) {
    ...
    const makeSwitchHanlder = ({ title, state, setState }, i) => {
        ...
        const makeSwitch = (i) => (
            <Switch key={i} onClick={switchHandler(i)} state={state == i} />
        )
    ...
    const startOrStopButton = running ? (
        <Button type="danger" onClick={toggleRunning}>
            STOP
        </Button>
    ) : (
        <Button type="success" onClick={toggleRunning}>
            START
        </Button>
    )
    ...
    return (
        <>
            ...
            <FlexRow wrap="wrap">
                <FlexRow>{axis.map(makeSwitchHanlder)}</FlexRow>
