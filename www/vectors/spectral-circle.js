...
import { simpleHarmonicMotionCos, simpleHarmonicMotionSin, fourier, squareWaveSequenceSin, squareWaveSequenceCos, random } from "../utils/maths-tools.js"
...
class SpectralCircle {
  ...
  update() {
    const { svg, props: { width, height, data, amplitude, offset, frequency, multiplierX, multiplierY, omega, numbers } } = this
    ...
    const squarewaveTransformX = (time) => fourier(radiusX, omega, time, numbers, squareWaveSequenceSin)

    const xOrigin = i => boxWidth * (i % xCount) + radiusX
    const x = (d,i) => d[0] == 0
      ? xOrigin(i) + Math.floor(squarewaveTransformX(multiplierX * (d[1] + offset)))
      : xOrigin(i)


    const squarewaveTransformY = (time) => fourier(radiusY, omega, time, numbers, squareWaveSequenceSin)

    const yOrigin = i => boxHeight * Math.ceil( (1 + i) / xCount)
    const y = (d,i) => d[0] == 1
      ? yOrigin(i) + Math.floor(squarewaveTransformY(multiplierY * (d[1] + offset)))
      : yOrigin(i)
    ...
    const rx = (d) => d[0] == 1
      ? 1.9 * radiusX * (Math.floor(squarewaveTransformX(2 * (offset + d[1] ))) / radiusX)
      : radiusX
    ...
    svg.selectAll('ellipse')
      ...
      .attr("ry", ry)
      .attr("rx", rx)
      .attr("fill", (d, i) => 0 ? "red" : getSpectrumPosition(((i % xCount) + Math.ceil(i / xCount)) * 0.19))
  }
